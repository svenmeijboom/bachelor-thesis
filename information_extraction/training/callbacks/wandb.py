...
class WandbCallback(BaseCallback):
    ...
    def on_validation_end(self, step_num: int, results: EvaluationResult):
        wandb.log({f'val/{metric}': value for metric, value in results.flattened_metrics.items()}, step=step_num)

    def on_evaluation_end(self, identifier: str, results: EvaluationResult):
        summary_dict = {f'{identifier}/{metric}': value for metric, value in results.flattened_metrics.items()}
        tables = {}
